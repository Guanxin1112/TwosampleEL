#two_emplkdown1 is Newton down-hill method using relmo to calculate lambda and 
#estimate theta_EL,Delta_EL at the same time
two_emplkdown1<-function(beta,Rd,prob,h){
  svdtol = 1e-9
  thetap = beta[1]
  Deltap = beta[2]
  y = Rd$Y
  ...
  while(step < max_step){
    ...
    NWT = Two_Grad(beta,nlam,Rd,prob,h)
    grad = NWT$grad
    hess = NWT$hess
    ...
    for(j in 1:length(gradient_wts)){
      ...
      Deltanew = betanew[2]
      znew =  G1.fun(thetanew,Deltanew,Rd,prob,h)
    ...
    if(likhd1 < likhd0) break  
      ...
      #likhd0 = likhd1; unnecessary since we have updated 'beta'
      }
  }
  #cat("ninner=",ninner,"\n")
  #cat("step=",step,"\n")
...
Two_Grad<-function(beta,lambda,Rd,prob,h){
  theta = beta[1]
  Delta = beta[2]
  grad = rep(NA,2)
  ...
  dg1 = dG1.fun(theta,Delta,Rd,prob,h)   #\partial G / \partial theta
  ddg1 = ddG1.fun(theta,Delta,Rd,prob,h)  #\partial^2 G / \partial theta^2

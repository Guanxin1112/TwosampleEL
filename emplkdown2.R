emplkdown2<-function(theta,Delta,Rd,prob,h){
  theta0 = theta
  #likhd0 = -1e7
  step = 0
  max_step = 100
  ...
  while(step < max_step){
    ...
    NWT = DDtheta2(theta,Delta,nlam,Rd,prob,h)
    grad = NWT[1]
    hess = NWT[2]
    ninner = 0
    for(i in 1:length(gradient_wts)){
      thetanew = theta - gradient_wts[i]*grad/hess
      if(thetanew<=0) {thetanew=theta0;likhd1=likhd0;nlamnew = nlam;break}#thetanew=theta?
      znew =  G2.fun(thetanew,Delta,Rd,prob,h)
  ...
  return(cbind(likhd0,theta,nlam,step)) #not thetanew,nlamnew
}


#############################################################
#estimating function for Delta=Delta0
G02.fun<-function(theta,Delta,Rd,prob,h){
  sum(G2.fun(theta,Delta=Delta0,Rd,prob,h))
} 

#hession matrix
DDtheta2<-function(theta,Delta,lambda,Rd,prob,h){
  n = length(Rd$X)
  y = Rd$Y
  d = Rd$d
  g1 = G2.fun(theta,Delta,Rd,prob,h)
  dg1 = dG2.fun(theta,Delta,Rd,prob,h)
  ddg1 = ddG2.fun(theta,Delta,Rd,prob,h)
...
G2.fun<-function(theta,Delta,Rd,prob,h){
  X = Rd$X
  A = Rd$A
  delta = Rd$delta
  n = length(X)
...
dG2.fun<-function(theta,Delta,Rd,prob,h){
  X = Rd$X
  A = Rd$A
  delta = Rd$delta
  n = length(X)
...
ddG2.fun<-function(theta,Delta,Rd,prob,h){
  X = Rd$X
  A = Rd$A
  delta = Rd$delta
  n = length(X)
